...
- местный транспорт по НЗ (тарифы PTS-Logistics):
    NZ$0 Окленд → NZ$250 Хэмилтон/Вангарей → NZ$400 Тауранга →
    NZ$650 Нейпир → NZ$750 Палмерстон-Норт → NZ$790 Веллингтон →
    NZ$1,100 Крайстчерч (с паромом) → NZ$1,450 Данидин →
    NZ$1,700 Инверкаргилл.
    Если авто НЕ НА ХОДУ — транспорт ×2 (буксировка) + обязателен форклифт.
- инспекция (опционально) NZ$200,
- форклифт NZ$200 (обязателен для не на ходу),
