...
export default function EngagementPanel({
  ...
  if (bidsDisabled) {
    return (
      <div className="rounded-2xl border border-white/10 bg-[#0D111A] p-5" data-testid="engagement-panel-au-inquiry">
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0D111A] p-5" data-testid="engagement-panel-verified">
      <div className="flex items-start justify-between gap-3">
        <div>
        ...
        <span className="rounded-full bg-[#00C853]/15 px-2.5 py-1 text-[11px] font-bold text-[#00C853]">
          Депозит ✓
        </span>
      </div>
...
function OfferModal({ open, onClose, vehicle }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="relative w-full max-w-md rounded-2xl border border-white/10 bg-[#0D111A] p-6" onClick={(e) => e.stopPropagation()} data-testid="offer-modal">
        <button onClick={onClose} className="absolute right-3 top-3 rounded-lg p-2 text-gray-400 hover:bg-white/5">×</button>
        {done ? (
          <div className="py-6 text-center">
            <div className="mb-3 inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#00C853]/20 text-3xl">✓</div>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            {error && <div className="rounded-lg bg-[#FF3B30]/15 px-3 py-2 text-sm text-[#FF7A6F]" data-testid="offer-error">{error}</div>}
...
function BidModal({ open, onClose, vehicle, currentMaxNzd, onPlaced }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="relative w-full max-w-md rounded-2xl border border-white/10 bg-[#0D111A] p-6" onClick={(e) => e.stopPropagation()} data-testid="bid-modal">
        <button onClick={onClose} className="absolute right-3 top-3 rounded-lg p-2 text-gray-400 hover:bg-white/5">×</button>
        <form onSubmit={submit} className="space-y-4">
          <div>
          ...
          {error && <div className="rounded-lg bg-[#FF3B30]/15 px-3 py-2 text-sm text-[#FF7A6F]" data-testid="bid-error">{error}</div>}
          {success && <div className="rounded-lg bg-[#00C853]/15 px-3 py-2 text-sm text-[#7BE7A4]" data-testid="bid-success">{success}</div>}
